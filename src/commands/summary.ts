...
export async function runSummary(ctx: CommandContext): Promise<string> {
  ...
  const [monthlyRows, dailyRows, billing] = await Promise.all([
    source.fetchMonthly(principal, monthRange),
    source.fetchDaily(principal, dayRange),
  ...
  const modelRows = await source.fetchModels(principal, monthRange);
  ...
  const meta = buildReportMeta(ctx, principal, monthRange, billing);
  const totals = {
