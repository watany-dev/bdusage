...
const USERS_CONFLICTING_FILTERS =
  "users does not accept --principal, --principal-role, or --principal-tag; use --all only.";
...
export function assertUsersCommandOptions(ctx: CommandContext): void {
  ...
  if (ctx.options.principalArn || ctx.options.principalRole || ctx.options.principalTag) {
