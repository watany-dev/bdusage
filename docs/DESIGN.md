...
`doctor` は source / engine 単位で表示する。

```text
CUR actual
  DuckDB direct Parquet
    ✓ package: @duckdb/node-api available
    ✓ httpfs extension loadable
    ✓ S3 credentials resolved from profile default
    ✓ files matched: 42 parquet files
    ✓ required columns present
    ✓ Bedrock sample row found
    ✗ line_item_iam_principal populated
      Fix: enable caller identity allocation data in CUR 2.0 export

  Athena
    - skipped: no [cur.athena] config

Cost Explorer actual-lite
  ✓ getCostAndUsage allowed
```

`doctor --fix` は安全な修正だけ行う。
...
| 状況 | メッセージ方針 |
|------|----------------|
| DuckDB package なし | `Install optional dependency: @duckdb/node-api` |
| httpfs load 失敗 | `DuckDB httpfs extension could not be loaded` |
| S3 認証失敗 | profile / region / SSO login を案内 |
| files glob 0 件 | `cur.duckdb.files` の修正案を出す |
...
8. `init` で config discovery を追加する
