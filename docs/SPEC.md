...
| `ce` | actual-lite | Cost Explorer API | tag / account 依存 | v0.2 |
| `logs` | estimate | CloudWatch Logs Insights | IAM principal（ログ設定依存） | v0.3 |
| `metrics` | estimate/volume | CloudWatch Metrics | 不可（モデル別のみ） | 計画 |
| `auto` | — | cur を試行 → 失敗時 doctor | — | ✅ |
...
`--source auto` で cur が失敗した場合、v0.1 では ce に fallback せず `doctor` 実行を案内する。
