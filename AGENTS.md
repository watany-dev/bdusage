...
Commands (current): `summary`, `daily`, `monthly`, `models`, `whoami`, `doctor`, `today --source logs`. Use `--source cur|ce|auto` for billing; `--source logs` for estimates only. Use `--cur-engine auto|duckdb|athena` for CUR backend selection.
